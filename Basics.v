...
Definition blt_nat (n m : nat) : bool :=
  match n, m with
  | _,  _ => andb (ble_nat n m) (negb (beq_nat n m))
  end.
...
Proof.
  intros b c.
  destruct b.
  rewrite -> orb_true_true.
  intros H.
  rewrite <- H.
  reflexivity.
  rewrite -> andb_false_false.
  intros H.
  rewrite -> H.
  reflexivity.
  Qed.
...
Fixpoint inc_bin (n : bin) : bin :=
  match n with
  | zero => succ zero
  | twice n' => succ (twice n')
  | succ n' => twice n'
  end.

Example test_inc_bin1: (inc_bin zero) = (succ zero).
Proof. reflexivity. Qed.
Example test_inc_bin2: (inc_bin (succ zero)) = (twice zero).
Proof. reflexivity. Qed.
Example test_inc_bin3: (inc_bin (twice zero)) = succ (twice zero).
Proof. reflexivity. Qed.
Example test_inc_bin4: inc_bin (succ (twice zero)) = twice (twice zero).
Proof. reflexivity. Qed.

Fixpoint bin_to_nat (n : bin) : nat :=
  match n with
  | zero => O
  | succ n' => S (bin_to_nat n')
  | twice n' => S (S (bin_to_nat n'))
  end.

Example test_bin_to_nat0: (bin_to_nat zero) = 0.
Proof. reflexivity. Qed.
Example test_bin_to_nat1: (bin_to_nat (succ zero)) = 1.
Proof. reflexivity. Qed.
Example test_bin_to_nat2: (bin_to_nat (twice zero)) = 2.
Proof. reflexivity. Qed.
Example test_bin_to_nat3: (bin_to_nat (succ (twice zero))) = 3.
Proof. reflexivity. Qed.
Example test_bin_to_nat4: (bin_to_nat (twice (twice zero))) = 4.
Proof. reflexivity. Qed.
